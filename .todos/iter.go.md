...
## Correctness
- [ ] **`err == io.EOF` should be `errors.Is(err, io.EOF)`** (`iter.go:52`). An `IterFunc` that wraps
      ...
      The same bare comparison appears in `otel/event_store.go:166` and `:278`.
- [ ] **`it.err = nil` in the EOF branch is a redundant assignment** (`iter.go:54`) — `it.err` is
...
- [ ] **Not safe for concurrent use, and doesn't say so.** `current`, `err` and `done` are unsynchronised,
      and `NewSliceIterator`'s closure captures a plain `index`. One line of doc prevents a real bug.
- [ ] **`All` gives no way to distinguish "consumed nothing" from "already exhausted".** Calling it on
